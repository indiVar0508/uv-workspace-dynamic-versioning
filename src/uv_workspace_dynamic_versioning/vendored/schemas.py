...
@dataclass
class UvWorkspaceDynamicVersioning:
    vcs: Vcs = Vcs.Any
    ...
    def __post_init__(self):
        """Validate the UvWorkspaceDynamicVersioning configuration."""
        self._validate_vcs()
    ...
    @classmethod
    def from_dict(cls, data: dict) -> UvWorkspaceDynamicVersioning:
        """Create UvWorkspaceDynamicVersioning from dictionary with validation."""
        validated_data = _normalize(cls, data)
...
@dataclass
class Tool:
    uv_workspace_dynamic_versioning: UvWorkspaceDynamicVersioning | None = None

    def __post_init__(self):
        """Validate the Tool configuration."""
        if self.uv_workspace_dynamic_versioning is not None and not isinstance(
            self.uv_workspace_dynamic_versioning, UvWorkspaceDynamicVersioning
        ):
            raise ValueError(
                "uv-workspace-dynamic-versioning must be an instance of UvWorkspaceDynamicVersioning"
            )
    ...
    def from_dict(cls, data: dict) -> Tool:
        ...
        if "uv_workspace_dynamic_versioning" in validated_data and isinstance(
            validated_data["uv_workspace_dynamic_versioning"], dict
        ):
            validated_data["uv_workspace_dynamic_versioning"] = UvWorkspaceDynamicVersioning.from_dict(
                validated_data["uv_workspace_dynamic_versioning"]
            )
